...
ASS_TEMP_FILENAME = "preview.ass"
PREVIEW_IMAGE_FILENAME = "preview.png"
DEFAULT_BG_FILENAME = "default_bg.png"
DURATION = "0:00:00.00,0:00:05.00"
...
def generate_ass_file(style_str: str, preview_text: Tuple[str, Optional[str]]) -> str:
    ...
    dialogue = []
    if translate_text:
        dialogue.append(f"Dialogue: 0,{DURATION},Secondary,,0,0,0,,{translate_text}")
        dialogue.append(f"Dialogue: 0,{DURATION},Default,,0,0,0,,{original_text}")
    else:
        dialogue.append(f"Dialogue: 0,{DURATION},Default,,0,0,0,,{original_text}")
    ...
    temp_ass_path = Path(tempfile.gettempdir()) / ASS_TEMP_FILENAME
    temp_ass_path.write_text(ass_content, encoding="utf-8")
    return str(temp_ass_path)
...
def ensure_background(bg_path: Path) -> Path:
    ...
    if not bg_path or not bg_path.exists():
        bg_path = bg_path.parent / DEFAULT_BG_FILENAME
...
def generate_preview(style_str: str, preview_text: Tuple[str, Optional[str]], bg_path: str = "") -> str:
    ...
    bg_path = Path(bg_path) if bg_path else Path(__file__).parent / DEFAULT_BG_FILENAME
    bg_path = ensure_background(bg_path)

    output_path = Path(tempfile.gettempdir()) / PREVIEW_IMAGE_FILENAME
