...
from ..bk_asr.asr_data import ASRData, ASRDataSeg, from_srt
from ..utils.logger import setup_logger
from .split_by_llm import MAX_WORD_COUNT, split_by_llm
...
def merge_segments_based_on_sentences(
    segments: List[ASRDataSeg], sentences: List[str], max_unmatched: int = 5
) -> List[ASRDataSeg]:
    """
    基于提供的句子列表合并ASR分段
...
def split_long_segment(segs_to_merge: List[ASRDataSeg]) -> List[ASRDataSeg]:
    ...
    # 根据文本类型确定最大词数限制
    max_word_count = (
        MAX_WORD_COUNT_CJK if is_mainly_cjk(merged_text) else MAX_WORD_COUNT_ENGLISH
    )
    ...
    if count_words(merged_text) <= max_word_count or len(segs_to_merge) == 1:
        merged_seg = ASRDataSeg(
            merged_text.strip(), segs_to_merge[0].start_time, segs_to_merge[-1].end_time
        )
    ...
    n = len(segs_to_merge)
    gaps = [
        segs_to_merge[i + 1].start_time - segs_to_merge[i].end_time
        for i in range(n - 1)
    ]
...
def preprocess_segments(
    segments: List[ASRDataSeg], need_lower=True
) -> List[ASRDataSeg]:
    """
    预处理ASR数据分段:
    1. 移除纯标点符号的分段
...
def merge_by_time_gaps(
    segments: List[ASRDataSeg], max_gap: int = MAX_GAP, check_large_gaps: bool = False
...
def merge_common_words(segments: List[ASRDataSeg]) -> List[List[ASRDataSeg]]:
    ...
    for i, seg in enumerate(segments):
        ...
        max_word_count = (
            MAX_WORD_COUNT_CJK if is_mainly_cjk(seg.text) else MAX_WORD_COUNT_ENGLISH
        )
        if any(
            ...
            result.append(current_group)
            logger.debug(
                f"在前缀词 {seg.text} 前分割 - {''.join(seg.text for seg in current_group)}"
            )
            current_group = []
        ...
        if (
            ...
            result.append(current_group)
            logger.debug(
                f"在后缀词 {segments[i-1].text} 后分割 - {''.join(seg.text for seg in current_group)}"
            )
