...
from app.core.entities import (
    LANGUAGES, TranscribeModelEnum, TranscribeTask, SubtitleTask, SynthesisTask,
    TranscriptAndSubtitleTask, FullProcessTask,
    TranscribeConfig, SubtitleConfig, SynthesisConfig
)
...
class TaskFactory:
    ...
    def create_transcribe_task(
        ...
        if need_next_task:
            ...
            output_path = str(Path(cfg.work_dir.value) / file_name / "subtitle" / f"【原始字幕】{file_name}-{config.transcribe_model.value}.srt")
        else:
            output_path = str(Path(file_path).parent / f"【原始字幕】{Path(file_path).stem}-{config.transcribe_model.value}.srt")
    ...
    def create_subtitle_task(
        ...
        """创建字幕任务"""
        output_name = Path(file_path).stem.replace("【原始字幕】", "").replace(f"【下载字幕】", "")
        if need_next_task:
            output_path = str(Path(file_path).parent / f"【样式字幕】{output_name}-{cfg.model.value}.ass")
        else:
            output_path = str(Path(file_path).parent / f"【字幕】{output_name}-{cfg.model.value}.srt")
        ...
        config = SubtitleConfig(
            ...
            subtitle_style=TaskFactory.get_subtitle_style(cfg.subtitle_style_name.value),
            # 字幕分割
    ...
    def create_synthesis_task(
        ...
        """创建视频合成任务"""
        if need_next_task:
            output_path = str(Path(subtitle_path).parent.parent / f"【卡卡】{Path(video_path).stem}.mp4")
        else:
    ...
    def create_full_process_task(
        ...
        """创建完整处理任务（转录+字幕+合成）"""
        if output_path is None:
            output_path = str(Path(file_path).parent / f"{Path(file_path).stem}_final{Path(file_path).suffix}")
