...
def asr_page():
    ...
    with col1:
        ...
        if video_file:
            ...
            video_placeholder.video(video_file, subtitles=st.session_state.subtitle_path if st.session_state.subtitle_path else None)
            ...
            if st.session_state.srt_content and st.session_state.asr_data:
                ...
                with st.expander("📝 字幕预览", expanded=True):
                    ...
                    df = pd.DataFrame([{
                        ...
                        '时长(秒)': round((seg.end_time - seg.start_time) / 1000, 1),
                        '字幕文本': seg.text.strip()
